...
Either `-a` or `-o <Output file>` must be used to select the output destination for the tests. `-a` will append the results to the input file and `-o` will create or rewrite the destination file with the results.
