...
define check_files_regex
	@echo -e "$(BOLD)Checking $(1) file structure...$(NC)"
	@files_found=$$(find $(1) $(2) -type f $(3)); \
	if [ -n "$$files_found" ]; then \
		invalid_files=$$(echo "$$files_found" | grep -v -E "$(4)"); \
		if [ -n "$$invalid_files" ]; then \
			>&2 echo -e "$(BOLD)$(RED)Files in $(1) do not match the expected pattern $(4)"; \
			>&2 echo -e "$$invalid_files$(NC)"; \
			>&2 echo -e "$(BOLD)$(1) file structure failed...$(NC)"; \
			exit 2; \
		fi \
	else \
		>&2 echo -e "$(BOLD)$(RED)Files not found in $(1)$(NC)"; \
		>&2 echo -e "$(BOLD)$(1) file structure failed...$(NC)"; \
		exit 3; \
	fi
...
main_structure:
	@echo -e "$(BOLD)Checking $(MAIN_FOLDER) file structure...$(NC)"
	@files_found=$$(find $(MAIN_FOLDER) -maxdepth 1 -type f \( -name '*.h' -o -name '*.c' -o -name '*.cpp' \)); \
	if [ -n "$$files_found" ]; then \
		>&2 echo -e "$(BOLD)$(RED)Error: Found .h or .c or .cpp files in $(MAIN_FOLDER)"; \
		>&2 echo -e "$$files_found$(NC)"; \
...
iriocorecpp_include_base:
	@echo -e "$(BOLD)Checking $(IRIOCORECPP_INCLUDE_FOLDER) file structure...$(NC)"
	@files_found=$$(find $(IRIOCORECPP_INCLUDE_FOLDER) -type f ! -name '*.h'); \
	if [ -n "$$files_found" ]; then \
		>&2 echo -e "$(BOLD)$(RED)Error: Found other files than .h in $(IRIOCORECPP_INCLUDE_FOLDER) and their subfolders"; \
		>&2 echo -e "$$files_found$(NC)"; \
...
iriocorecpp_include_terminals_imp_equivalent_files:
	@echo -e "$(BOLD)Checking if the non Impl files exists in $(IRIOCORECPP_INCLUDE_TERMINALS_IMPL_FOLDER) file structure...$(NC)"
...
iriocorecpp_terminals_imp_equivalent_files:
	@echo -e "$(BOLD)Checking if the non Impl files exists in $(IRIOCORECPP_TERMINALS_IMPL_FOLDER) file structure...$(NC)"
