...
void findAndInsertEnumRegisters(const bfp::BFP &parsedBitfile, const std::string &terminalName, const size_t maxElem, std::unordered_map<std::uint32_t, const std::uint32_t> &mapInsert)
{
    findAndInsertEnumResources(parsedBitfile.getRegisters(), terminalName, maxElem, mapInsert);
}

void findAndInsertEnumDMAs(const bfp::BFP &parsedBitfile, const std::string &dmaName, const size_t maxElem, std::unordered_map<std::uint32_t, const std::uint32_t> &mapInsert)
{
                                        ...
                                        const std::string &resourceName)
{
	...
	if(it == mapResource.end()){
		throw std::runtime_error(std::to_string(n) + " is not a valid " + resourceName + " terminal");
	}
