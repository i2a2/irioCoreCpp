...
#include <terminals/namesTerminalsDigital.h>
#include <utils.h>

namespace iriov2{

    TerminalsDigital::TerminalsDigital(const bfp::BFP &parsedBitfile, const NiFpga_Session &session, Platform &platform):
        m_session(session)
    {
        //Find DI
    ...
    bool getDigital(const NiFpga_Session &session,
    ...
    void TerminalsDigital::setDO(const std::uint32_t n, const bool value) const
    {
        ...
        throwIfNotSuccessNiFpga(status, "Error writing terminal "
                + std::string(TERMINAL_DO) + std::to_string(n));
    }
}