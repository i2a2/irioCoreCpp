...
TerminalsDMADAQImpl::TerminalsDMADAQImpl(
		...
		const std::string &nameTermDMAEnable) :
				TerminalsDMACommonImpl(parsedBitfile, session, platform, nameTermNCh, nameTermFrameType,
...
std::uint16_t TerminalsDMADAQImpl::getLengthBlock(const std::uint32_t &n) const {
